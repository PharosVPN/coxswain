...
type Relay struct {
	...
	OnionPubKey   string
	Status        string
	// AgentVersion is the relay build last deployed to this relay (mirrors
	// Node.AgentVersion). Empty for the embedded relay and until first recorded.
	AgentVersion string
...
const relayColumns = `id, name, kind, region, endpoint, egress_endpoint, egress_hop, onion_endpoint, onion_pubkey, status, agent_version, version, created_at, updated_at, server_id`
...
func CreateRelay(ctx context.Context, db *sql.DB, r Relay) (Relay, error) {
	...
	_, err := db.ExecContext(ctx,
		...
		r.OnionEndpoint, r.OnionPubKey, r.Status, r.AgentVersion, r.Version, r.CreatedAt, r.UpdatedAt, r.ServerID)
	if err != nil {
...
func UpdateRelay(ctx context.Context, db *sql.DB, r Relay) (Relay, error) {
	...
	res, err := db.ExecContext(ctx,
		`UPDATE relays SET name = ?, kind = ?, region = ?, endpoint = ?, egress_endpoint = ?, egress_hop = ?,
		        onion_endpoint = ?, onion_pubkey = ?, status = ?, agent_version = ?, server_id = ?,
		        version = version + 1, updated_at = ?
		 WHERE id = ? AND version = ?`,
		r.Name, r.Kind, r.Region, r.Endpoint, r.EgressEndpoint, r.EgressHop,
		r.OnionEndpoint, r.OnionPubKey, r.Status, r.AgentVersion, r.ServerID, now, r.ID, r.Version)
	if err != nil {
